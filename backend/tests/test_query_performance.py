...
def assert_indexed(stages, label):
    assert "COLLSCAN" not in stages, (
        f"{label} reads the whole collection: {' -> '.join(stages)}"
    )
    assert any("IXSCAN" in stage for stage in stages), (
...
class TestSigningIn:
    ...
    def test_finding_an_account_by_email_uses_an_index(self, mongo_db, populated):
        ...
        stages, stats = explain(
            mongo_db.users.find({"email": populated["sample_email"]})
        )
    ...
    def test_looking_up_a_session_uses_an_index(self, mongo_db, populated):
        stages, stats = explain(
            mongo_db.user_sessions.find({"session_token_hash": "a" * 64})
        )
...
class TestCatalogue:
    def test_browsing_a_category_uses_an_index(self, mongo_db, populated):
        cursor = (
            mongo_db.products.find({"category": "laundry"})
    ...
    def test_sorting_the_catalogue_is_served_by_an_index(
        self, mongo_db, populated, sort_name
    ):
    ...
    def test_reviews_for_a_product_use_an_index(self, mongo_db, populated):
        cursor = (
            mongo_db.reviews.find({"product_id": "prod_missing"})
            .sort([("created_at", DESCENDING)])
        )
...
class TestKnownInMemorySorts:
    ...
    def test_the_date_ordering_still_reaches_rows_through_an_index(
        self, mongo_db, populated
    ):
        cursor = (
            mongo_db.products.find({"featured": True})
            .sort(PRODUCT_SORTS["popular"])
            .limit(12)
        )
        ...
        assert stats["totalDocsExamined"] < 1000, (
