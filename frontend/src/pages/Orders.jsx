    ...
    <div className="max-w-5xl mx-auto px-6 py-10" data-testid="orders-page">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-16 rounded-2xl border border-[var(--nayara-border)] bg-white">
          <Package className="w-14 h-14 mx-auto text-[#5C7671] mb-4" />
          <p className="text-[#5C7671]">No orders yet.</p>
          ...
          {orders.map((o) => (
            <div key={o.order_id} className="rounded-2xl border border-[var(--nayara-border)] bg-white p-6" data-testid={`order-${o.order_id}`}>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div>
                  <div className="text-xs text-[#5C7671]">Order #{o.order_id}</div>
                  <div className="text-xs text-[#5C7671]">{new Date(o.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</div>
                </div>
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                {o.items.slice(0, 5).map((it) => (
                  <img key={it.product_id} src={it.image} alt="" className="w-14 h-14 rounded-lg object-cover bg-[#F0F5F3]" />
                ))}
                {o.items.length > 5 && <div className="w-14 h-14 rounded-lg bg-[#F0F5F3] flex items-center justify-center text-xs">+{o.items.length - 5}</div>}
              </div>
              <div className="flex justify-between items-center pt-4 border-t border-[var(--nayara-border)]">
                <div className="text-sm text-[#5C7671]">{o.items.length} item(s)</div>
